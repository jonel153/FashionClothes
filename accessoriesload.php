...
$result = mysql_query('SELECT * FROM product WHERE category="Accessories" AND prodID IN(SELECT prodID FROM topsales)');
...
$count = 1;
	while($row = mysql_fetch_array($result)){
		...
		echo "<div class='col-xs-4'>
			...
			echo "<a href=\"view.php?id=\"><img class='img-thumbnail' src='".$row[3]."'> <br /><br />";
			echo "<label class='name'>".$row[1]."</label><br />";
			echo "<label class='payment'>&#8369;".$row[2]."</label></a>";
			echo "</div> </div> </div>";
